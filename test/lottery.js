...
contract('Lottery', (accounts) => {
  ...
  it('Should NOT create bet if not admin', async () => {
    await expectRevert(
      lottery.createBet(3, 10, {from: accounts[1]}),
      "only admin"
    )
  });
  ...
  it('Should NOT create bet if state not idle', async () => {
    ...
    await lottery.createBet(5, 2);
    await expectRevert(
      lottery.createBet(3, 10),
      "current state does not allow this"
    )
  });

  it('Should create a bet', async () => {
    await lottery.createBet(3, 10);
  ...
  it('Should NOT bet if not sending exact bet amount', async () => {
    ...
    await lottery.createBet(3, 10)
    await expectRevert(
      lottery.bet({from: accounts[1], value: 11}),
      "can only bet exactly the bet size"
    )
  });

  it('Should bet', async () => { //NEED TO GO OVER DO NOT UNDERSTAND
  ...
  it('Should NOT cancel if not admin', async () => {
    await lottery.createBet(3, 10, {from: accounts[0]});
